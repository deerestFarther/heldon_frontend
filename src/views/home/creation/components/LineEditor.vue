...
export default {
  ...
  methods: {
    ...
    async confirmChangeLine (line) {
      ...
      await axios.put('http://116.62.36.50:8080/relation/updateRelation', this.curLine)
          .then(({ data }) => {
            if (data) {
              this.$message({
                type: 'success',
    ...
    deleteLine (lineId) {
      this.$confirm('此操作将永久删除该关系，是否继续？', '提示', {
        ...
        type: 'warning'
            ...
            axios.delete('http://116.62.36.50:8080/relation/deleteRelationByEdgeId/' + lineId)
                .then(({ data }) => {
                  if (data) {
    ...
    addNewLine () {
      this.$refs.newLineForm.validate((valid) => {
        if (valid) {
          ...
          axios.post('http://116.62.36.50:8080/relation/insertRelation', this.newLineForm)
              .then(({ data }) => {
                if (data) {
                  this.$emit('lineUpdated')
