...
export default {
  ...
  methods: {
    ...
    async confirmChangeNode (node) {
      ...
      await axios.put(this.serverUrl + 'node/updateNode', this.curNode)
          .then((data) => {
            console.log(data)
          })
          .catch((err) => {
    ...
    deleteNode (node) {
      this.$confirm('此操作将永久删除该该结点和与之相关的关系，是否继续？', '提示', {
        ...
        type: 'warning'
            ...
            axios.delete(this.serverUrl + 'node/deleteNodeByNodeId/' + node.data.id)
                .then(({ data }) => {
                  console.log(data)
    ...
    addNewNode () {
      this.$refs.newNodeForm.validate((valid) => {
        if (valid) {
          ...
          axios.post(this.serverUrl + 'node/insertOneNode', this.newNodeForm)
              .then(({ data }) => {
                if (data) {
