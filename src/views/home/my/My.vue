...
export default {
  ...
  return temp;
    ...
    getUser(){
      ...
      axios.get(this.serverUrl + "authorization/get/authorizations/"+this.userId).then(({data})=>{
        if (data) {
      ...
      axios.get(this.serverUrl + "user/getInfoByUserId/"+this.userId).then(({data})=>{
        if (data) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          console.log(list)
          axios.post(this.serverUrl + "user/edit",list).then(({data})=> {
            console.log(data)
            if(data) {
              this.$notify({
                title: '成功',
    ...
    submitForm1(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          console.log(list1)
          axios.post(this.serverUrl + "user/edit",list1).then(({data})=> {
            console.log(data)
            if(data) {
              this.$notify({
                title: '成功',
