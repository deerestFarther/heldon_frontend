  ...
  data() {
    var checkAccount = (rule, value, callback) => {
      ...
      }else if(value.length < 2 || value.length > 6)
      {
        ...
        axios.get(this.serverUrl + "authorization/exists/authorization/"+this.ruleForm.account).then(({data})=>{
          console.log(data)
          if (!data) {
            return callback()
          } else {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post(this.serverUrl + "authorization/add/authorization/new/"+"0"+"&&"+this.ruleForm.account
